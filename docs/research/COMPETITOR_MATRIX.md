...
| **memstechtips/Winhance** | Excellent integrated WPF customization UX; good grouping of Windows features; "remove vs disable" distinction | **Licence has a no-compete/redistribution restriction** → behaviour & UX study only, reimplement from MS docs | Restricted — **inspiration only** |
...
| **farag2/Sophia-Script / SophiApp** | Gold standard for *documented, reversible* functions;每 function has a clear on/off; huge coverage; version-gated | Sheer surface area; SophiApp rewrite churn; some items niche | MIT |
...
| **Sysinternals Process Explorer** | Process tree; DLL/handle lower pane; verify signatures; colour legend; "find window's process" | Tree + lower-pane pattern → process inspector |
