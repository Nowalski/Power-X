...
| `privacy.advertising-id` | `AdvertisingInfo\Enabled=0` | MS "Manage connections…" doc §Advertising ID | Low, **Recommended** |
...
## Refused (folklore / harmful / unsubstantiated) — prompt §17/§18
| Claim | Verdict |
|---|---|
...
1. State the mechanism and the Microsoft/《winternl》/GPO source.
2. Define the build range and edition constraints.
3. Benchmark per `BENCHMARK_PLAN.md` (before/after, ≥5 runs, report variance).
4. If Δ is within noise → keep it but label "no measurable performance benefit on test hardware; may help niche cases".
