...
version 31.0.15.3623, and that module is on the faulting thread's stack" → *likely an NVIDIA
...
- Do **not** parse dumps found via a user-supplied arbitrary path without the same
  size/section sanity checks as any file parser (see §5).
...
- A minidump's stack memory can contain **secrets** — passwords, tokens, file contents that
  ...
  (redact `\Users\<name>\` → `\Users\<user>\`, drop environment blocks, drop the memory
  streams).
