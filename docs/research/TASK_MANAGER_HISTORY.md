...
Sources: Microsoft "Building Windows 8" blog (the Task Manager redesign posts by the In-The-Box / Fundamentals team, 2011), subsequent Windows blog posts, and observable behaviour of Win8→Win11 Task Manager.
...
| **Friendly names + icons** instead of `svchost.exe ×12` | Group svchost by hosted services; show the service list in the inspector |
...
| Per-app **history** (Alt+tab-style resource usage over time for Store apps) | Resource timeline feature idea (§68) |

## Later evolution (Win10 → Win11)
...
4. **The tool runs when the PC is already sick** — cheapness and resilience are features (§38, §76).
